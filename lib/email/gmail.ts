...
async function validateRefreshTokenMatchesEmail(refreshToken: string, expectedEmail: string) {
  ...
  try {
    ...
    throw new Error(
      `Failed to refresh access token from refresh token: ${msg}. This usually means the refresh token is revoked or was issued to a different OAuth client.`
    );
  }
  ...
  if (tokenEmail.toLowerCase() !== expectedEmail.toLowerCase()) {
    throw new Error(
      `Refresh token belongs to ${tokenEmail} but you attempted to send as ${expectedEmail}. Use a refresh token for ${expectedEmail} or set GMAIL_USER to ${tokenEmail}.`
    );
  }
 ...
 */
export async function createTransporter(opts?: {
  refreshToken?: string;
  userEmail?: string;
  ...
  if (opts?.refreshToken) {
    ...
    } catch (perUserErr: unknown) {
      // if caller allows fallback, continue to try env token; otherwise rethrow with message
      if (!opts?.allowSystemFallback) {
        const m = perUserErr instanceof Error ? perUserErr.message : String(perUserErr);
        throw new Error(`Per-user refresh token validation failed: ${m}`);
      }
      console.warn(
        "[Gmail] Per-user refresh token validation failed; will attempt system token (if present):",
  ...
  if (envRefresh) {
    try {
      ...
      throw new Error(`System refresh token validation failed: ${m}`);
    }
  }

  // no tokens at all
  throw new Error(
    "No refresh token available. Provide a per-user refresh token or set GMAIL_REFRESH_TOKEN (or REFRESH_TOKEN). Ensure your OAuth flow requested access_type=offline and included the Gmail scope (https://www.googleapis.com/auth/gmail.send)."
  );
}
...
export async function sendEmail(
  ...
  try {
    ...
    console.error("[Gmail] sendEmail error:", err instanceof Error ? err.message : String(err));
    return false;
  }
}
...
export async function testGmailConnection(opts?: { refreshToken?: string; userEmail?: string }): Promise<boolean> {
  try {
    ...
    console.error("[Gmail] testGmailConnection failed:", error instanceof Error ? error.message : String(error));
    return false;
  }
}
